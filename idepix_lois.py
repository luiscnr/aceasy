...
class IDEPIX:
    ...
    def __init__(self, fconfig, verbose):
        ...
        if os.path.exists(fconfig):
            ...
            if options.has_section('IDEPIX'):
                if options.has_option('IDEPIX', 'gpt_path'):
                    self.gpt_path = options['IDEPIX']['gpt_path']

    def check_runac(self):
        if self.gpt_path is None:
            if self.verbose:
                print('[ERROR]: IDEPIX class can no be started. GPT file path is not available')
            return False
        if not os.path.exists(self.gpt_path):
            if self.verbose:
                print('[ERROR]: IDEPIX class can no be started. GPT path does not exist')
            return False
        return True
    ...
    def run_process(self, prod_path, output_dir):
        ...
        if prod_name.endswith('.SEN3') and os.path.isdir(prod_path):
            output_name = prod_name[0:-5] + '_IDEPIX.nc'
        ...
        if proc.returncode == 0:
            if self.verbose:
                print(f'[INFO] IDEPIX-OLCI completed. Output file name: {output_name}')
            return True
        else:
            print(f'[ERROR] IDEPIX-OLCI  NOT completed for product: {prod_name}')
            return False